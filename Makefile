...
# project name
PROJECT = lpc1114_freertos

# core type
CORE = cortex-m0

# linker script
LD_SCRIPT = ./linker/lpc1114_flash_crp.ld
...
# flags for C compiler
C_FLAGS = -std=$(C_STD) -g -ggdb3 -fverbose-asm -Wa,-ahlms=$(OUT_DIR_F)$(notdir $(<:.$(C_EXT)=.lst)) -D__USE_CMSIS 

# flags for assembler
AS_FLAGS = -g -ggdb3 -Wa,-amhls=$(OUT_DIR_F)$(notdir $(<:.$(AS_EXT)=.lst))

# flags for linker
LD_FLAGS = -T$(LD_SCRIPT) -g -Wl,-Map=$(OUT_DIR_F)$(PROJECT).map,--cref,--no-warn-mismatch,--allow-multiple-definition --specs=nano.specs -lc -lnosys
